...
import { useAnimations, useDeletionRequests, useParticipantTimeCorrectionRequests, useTimeCorrectionRequests } from '@/hooks/queries/useAnimations'
import {
  useValidateAnimation, useRejectAnimation, useApproveDeletion, useDenyDeletion,
  useApproveTimeCorrection, useDenyTimeCorrection,
  useApproveParticipantTimeCorrection, useDenyParticipantTimeCorrection,
} from '@/hooks/mutations/useAnimationMutations'
import type { AnimationStatus, DeletionRequest, ParticipantTimeCorrectionRequest, TimeCorrectionRequest } from '@/types/database'
...
function ParticipantTimeCorrectionRequestCard({ request }: { request: ParticipantTimeCorrectionRequest }) {
  const { mutate: approve, isPending: approving } = useApproveParticipantTimeCorrection()
  const { mutate: deny, isPending: denying } = useDenyParticipantTimeCorrection()
  const anim = request.animation
  const participantUser = request.participant?.user ?? request.requester

  const handleApprove = () => {
    if (!confirm(`Appliquer la correction d'inscription pour "${participantUser?.username ?? 'ce participant'}" ?`)) return
    approve(request.id, {
      onSuccess: () => toast.success("Correction d'inscription appliquée."),
      onError: (err) => toast.error(err instanceof Error ? err.message : 'Erreur'),
    })
  }
    ...
    <GlassCard className="p-5 border-cyan-500/10">
      <div className="flex items-start justify-between gap-3 mb-3">
        <div className="flex-1 min-w-0">
          ...
          {participantUser && (
            <div className="flex items-center gap-1.5 mt-1">
              <UserAvatar avatarUrl={participantUser.avatar_url} username={participantUser.username} size="xs" />
              <span className="text-xs text-white/40">Demandé par {participantUser.username}</span>
            </div>
          )}
        </div>
        {anim && <StatusBadge status={anim.status} />}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs mb-3">
        <div className="rounded-lg border border-white/[0.06] bg-white/[0.03] p-3">
          ...
          <p className="text-white/60">
            Inscription : {request.participant?.joined_at
              ? formatDateTime(request.participant.joined_at)
              : request.current_joined_at
                ? formatDateTime(request.current_joined_at)
                : '—'}
          </p>
          <p className="text-white/60">Début animation : {anim?.started_at ? formatDateTime(anim.started_at) : '—'}</p>
        </div>
        <div className="rounded-lg border border-cyan-500/20 bg-cyan-500/10 p-3">
          <p className="text-cyan-300 uppercase tracking-wider font-semibold mb-2">Demandé</p>
          <p className="text-white/75">Inscription : {formatDateTime(request.requested_joined_at)}</p>
          <p className="text-white/75">Fin animation : {anim?.ended_at ? formatDateTime(anim.ended_at) : '—'}</p>
        </div>
      </div>
...
export default function Validation() {
  ...
  const [tab, setTab] = useState<'pending' | 'open' | 'rejected' | 'deletion' | 'time' | 'participant-time'>('pending')
  ...
  const { data, isLoading } = useAnimations(
    tab !== 'deletion' && tab !== 'time' && tab !== 'participant-time' ? { status: statusMap[tab as 'pending' | 'open' | 'rejected'] } : {},
  )
  ...
  const { data: participantTimeCorrectionData, isLoading: participantTimeCorrectionLoading } = useParticipantTimeCorrectionRequests(canManageFullValidation)

  const animations = tab !== 'deletion' && tab !== 'time' && tab !== 'participant-time' ? (data?.animations ?? []) : []
  ...
  const participantTimeCorrectionRequests = participantTimeCorrectionData?.requests ?? []
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto space-y-6">
      ...
      <Tabs value={tab} onValueChange={(v) => setTab(v as typeof tab)}>
        <TabsList>
          ...
          {canManageFullValidation && (
            <>
              ...
              <TabsTrigger value="participant-time" className="relative">
                Inscriptions
                {participantTimeCorrectionRequests.length > 0 && (
                  <span className="ml-1.5 rounded-full bg-cyan-500/20 text-cyan-300 text-xs px-1.5 py-0.5 leading-none">
                    {participantTimeCorrectionRequests.length}
                  </span>
                )}
              </TabsTrigger>
            </>
        ...
        {canManageFullValidation && (
          <TabsContent value="participant-time">
            {participantTimeCorrectionLoading ? (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {[...Array(2)].map((_, i) => <Skeleton key={i} className="h-56" />)}
              </div>
            ) : participantTimeCorrectionRequests.length === 0 ? (
              <GlassCard className="p-12 text-center">
                <p className="text-white/30 text-sm">Aucune demande de correction d'inscription</p>
              </GlassCard>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {participantTimeCorrectionRequests.map((request) => (
                  <ParticipantTimeCorrectionRequestCard key={request.id} request={request} />
                ))}
              </div>
            )}
          </TabsContent>
